    ...
    <el-submenu>
      ...
      <el-form ref="titleForm" :model="bodyForm" label-position="right" label-width="80px" size="small">
        ...
        <el-form-item label="柱形宽度">
          <div class="form_item">
            <el-slider v-model="bodyForm.barWidth" :min="40" :max="100" @input="get_barWidth"
                       :format-tooltip="(value) => {return value + '%'}"
                       :show-input="true" class="font_size"></el-slider>
          </div>
        </el-form-item>
        <el-form-item label="显示背景">
          <el-switch v-model="bodyForm.showBackground" active-text="显示" inactive-text="关闭">
          </el-switch>
        </el-form-item>
        <el-form-item label="背景颜色">
          <div class="form_item">
            <el-input prefix-icon="iconfont icon-huitutiaosepan" v-model="bodyForm.backgroundStyle.color"></el-input>
            <el-color-picker v-model="bodyForm.backgroundStyle.color"
                             @active-change="change_color_bg"></el-color-picker>
          </div>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    change_color_bg(color) {
      this.bodyForm.backgroundStyle.color = color
    },
