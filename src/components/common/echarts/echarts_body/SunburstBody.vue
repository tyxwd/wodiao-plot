    ...
    <el-submenu>
      ...
      <el-form ref="bodyForm" :model="bodyForm" label-position="right" label-width="80px" size="small">
        ...
        <el-form-item label="图形颜色">
          ...
          <p style="color: #ff5777">注：目前旭日图不支持分组调色</p>
        </el-form-item>
        ...
        <el-form-item label="圆环内径">
          <el-slider v-model="bodyForm.radius_[0]" :min="0" :max="100" :step="1" @input="get_innerRadius"
                     :format-tooltip="(value) => {return value + '%'}"
                     show-input class="font_size"></el-slider>
        </el-form-item>
        <el-form-item label="圆环外径">
          <el-slider v-model="bodyForm.radius_[1]" :min="0" :max="100" :step="1" @input="get_outerRadius"
                     :format-tooltip="(value) => {return value + '%'}"
                     show-input class="font_size"></el-slider>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    change_body_option() {
      this.$store.state.chart_option.option.series = this.bodyForm
    ...
    get_innerRadius(value) {
      this.$store.state.chart_option.option.series.radius[0] = value + "%"
      this.$store.commit("change_chart_time")
    },
    get_innerBorderRadius(value) {
      this.$store.state.chart_option.option.series.itemStyle.borderRadius[0] = value + "%"
      this.$store.commit("change_chart_time")
    },
    get_outerRadius(value) {
      this.$store.state.chart_option.option.series.radius[1] = value + "%"
      this.$store.commit("change_chart_time")
    },
    get_outerBorderRadius(value) {
      this.$store.state.chart_option.option.series.itemStyle.borderRadius[1] = value + "%"
      this.$store.commit("change_chart_time")
    },
    resetForm() {
  ...
  mounted() {
    sleep(50).then(() => {
      this.$store.state.chart_option.option.series.radius = [0, '90%']
      this.$store.commit("change_chart_time")
    })
  },
  watch: {
